...
### 核心功能
- **用户认证系统**: JWT + bcrypt 安全认证
- **服务器管理**: 创建、编辑、删除、启动、停止ARK服务器
- **Docker容器化**: 每个服务器运行在独立的Docker容器中
- **配置文件管理**: 可视化编辑Game.ini和GameUserSettings.ini
- **RCON管理**: 服务器远程控制台功能
- **实时状态监控**: 服务器状态实时更新
- **镜像下载进度**: 实时显示Docker镜像下载进度和状态

### 镜像管理功能
- **智能镜像检查**: 自动检查必要的Docker镜像是否存在
- **异步镜像下载**: 后台异步下载镜像，不阻塞系统启动
- **实时进度显示**: 显示每个镜像的下载进度百分比
- **详细状态信息**: 显示镜像下载的具体阶段（下载中、解压中、验证中等）
- **总体进度统计**: 显示所有镜像的总体下载进度
- **自动状态更新**: 每2秒自动刷新镜像状态
- **用户友好界面**: 直观的进度条和状态指示器

### 技术特性
- **前后端分离**: Nuxt.js 3 + Gin 架构
- **响应式设计**: 支持桌面和移动设备
- **实时更新**: WebSocket风格的轮询更新
- **错误处理**: 完善的错误提示和恢复机制
